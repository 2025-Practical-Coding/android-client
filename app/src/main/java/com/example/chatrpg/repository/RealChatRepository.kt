...
class RealChatRepository : ChatRepository {
    ...
    private fun parseDynamicChatResponse(response: Response<ResponseBody>): Any {
        ...
        return when {
            jsonElement.isJsonArray -> gson.fromJson(raw, Array<ChatResponse>::class.java).toList()

            jsonElement.isJsonObject -> {
                val obj = jsonElement.asJsonObject
                when {
                    obj.has("game_over") -> gson.fromJson(raw, GameResultResponse::class.java)
                    obj.has("reply") -> gson.fromJson(raw, ChatResponse::class.java)
                    else -> throw Exception("예상치 못한 JSON 응답: $raw")
                }
            }
